...
const Verifier = ({
  ...
  const [itemsPerPage] = useState(8); // You can adjust this based on your needs
  ...
  }, [router.query.currentPage, verifiedProperties, router.query.selectedDate]);

  useEffect(() => {
    setFilteredProducts(verifiedProperties);
  ...
  }, [verifiedProperties, initialSearchTerm, router.query]);
  ...
  const filterProducts = () => {
    ...
    const filteredData = verifiedProperties.filter((product) => {
      const propertyName = product.propertyId.propertyName
  ...
  const handleSeeMore = (verifierId) => {
  ...
  const totalfiltertedProducts = filteredProducts.length;
                ...
                <div className="flex flex-col">
                  <div className="mt-12">
                    <button
                      ...
                      onClick={() => handleSeeMore(verifierId)} // Assuming the sellerId is present in the products array
                    >
                      See More
                    </button>
                  </div>
